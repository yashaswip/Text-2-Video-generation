...
def main():
    ...
    # Load the pre-trained CLIP model, use CPU if no GPU is available
    device = "cuda" if torch.cuda.is_available() else "cpu" # Use CPU if no GPU
    model, preprocess = clip.load("ViT-B/32", device=device) 

    # Directory containing video files
    directory = r"/content/drive/MyDrive/outputs"
    
    # CSV file to save results
    csv_file = r"/content/drive/MyDrive/outputs/results.csv"
        ...
        for root, _, files in os.walk(directory):
            for filename in files:
                if filename.endswith(".gif"):
                    ...
                    clip_score = get_clip_score(image, prompt, model, preprocess, device) # Pass the determined device
